...
class FlightController extends Controller
{
  ...
  public function flight(Request $request)
  {
    ...
    $locationIP = $request->ip();
    $currentUserInfo = Location::get($locationIP);
    $CurrentCountry = isset($currentUserInfo->countryName)?$currentUserInfo->countryName:'';
    $CurrentCurrency = isset($currentUserInfo->countryCode)?$currentUserInfo->countryCode:'';
  ...
  public function searchFlight(Request $request)
  {
    ...
    $currencyCode = $request->get('currentCurrency');
    ...
    if (!array_key_exists("CurrentCountry", $cookie)) {
      setcookie("CurrentCurrency", $CurrentCurrency, time() + 3600);
      setcookie("CurrentCountry", $CurrentCountry, time() + 3600);
    }
  ...
  public function roundtripsearch(Request $request)
  {
    ...
    $RoundFlightFromDate = $carbonDate->format('Y-m-d');
    ...
    $RoundFlightReturnDate = $carbonDate->format('Y-m-d');
    ...
    $url = 'https://test.api.amadeus.com/v2/shopping/flight-offers';
    $queryParams = [
      'originLocationCode'      => $RoundtripFlightAirportCodeFrom,
      'destinationLocationCode' => $RoundtripFlightAirportCodeTo,
      'departureDate'           => $RoundFlightFromDate,
      'returnDate'              => $RoundFlightReturnDate,
      'adults'                  => $RoundtripFlightAdultSlct,
      'children'                => $RoundtripFlightChildrenSlct,
      'infants'                 => $RoundtripFlightInfantSlct,
      'travelClass'             => $RoundtripFlightCoach,
      'nonStop'                 => "true",
      'max'                     => "250",
      'currencyCode' => $currencyCode,
    ];
